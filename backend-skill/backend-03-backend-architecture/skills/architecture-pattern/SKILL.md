...
description: 当需要选择和设计后端架构模式时使用。架构模式自动选择与设计，基于业务规模、团队能力和技术约束，自动评估并推荐后端架构模式（单体/微服务/Serverless），输出架构决策记录和系统拓扑图。关键词：架构模式、微服务、单体架构、Serverless、架构决策、系统拓扑、选架构、微服务拆不拆。
metadata:
  module: "后端架构与开发"
  sub-module: "后端架构"
  type: "pipeline"
...
- [ ] 架构推荐有据：量化评估支撑（非主观判断）
- [ ] ADR记录完整：每个关键决策有架构决策记录
- [ ] 演进路线可行：有明确的触发条件和时间预估
- [ ] 技术选型匹配：技术栈与架构模式一致
