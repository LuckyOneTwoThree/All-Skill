...
| 探索阶段设计决策 | JSON | ○ | output/ui-frontend/design-exploration/design_decisions.json | progressive模式下Stage 0.5约束对齐产出的设计决策，作为design_decisions初始值 |
...
**探索阶段决策消费规则**：
- 若有探索阶段设计决策输入（design_decisions.json from Stage 0.5）：其 decisions[] 作为 design_decisions 的初始值
...
- v1.7: 新增探索阶段设计决策输入（design_decisions.json from Stage 0.5），作为design_decisions初始值；PM约束偏离记录增加探索阶段决策消费规则
