...
description: 当需要自动执行验收并生成签收报告时使用。自动化验收执行+签收报告生成，基于Given-When-Then格式的验收标准自动执行验收（P0/P1失败阻断上线），整合测试结果、验收标准、遗留问题和签收确认，产出可签收的验收报告。🤖 AI自动执行+AI建议人类审批。关键词：自动化验收、验收测试、Given-When-Then、质量门禁、上线检查、验收报告、签收报告、UAT报告、验收确认。
metadata:
  module: "产品监控与迭代"
  sub-module: "质量保障"
  type: "pipeline"
...
| 文件 | 格式 | 说明 |
|------|------|------|
| acceptance-report.md | Markdown | 完整验收测试报告（含自动化执行结果和签收确认） |
