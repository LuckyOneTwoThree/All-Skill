---
name: design-prototype
description: 原型自动生成。基于IA方案和User Flow自动生成低保真和中保真原型，包含设计规范检查和可用性启发式评估。适用于快速原型验证和设计稿生成。关键词：原型设计、低保真原型、中保真原型、设计规范、原型生成。
metadata:
  module: "产品构思与设计"
  sub-module: "产品设计与原型"
  type: "pipeline"
  version: "1.0"
---

# Pipeline 11: 原型自动生成

## 核心原则

1. **批量生成人类筛选**：AI批量生成分类/排序建议，人类做最终筛选和判定
2. **结构化发散**：用固定模板和框架引导需求拆解，避免遗漏和随意性
3. **假设驱动而非功能驱动**：每个需求背后必须还原为用户假设，而非直接进入功能设计
4. **设计规范即约束**：需求分析阶段就引入设计规范约束，避免后期返工

🤖→👤 AI建议 → 人类审批

## 交互模式

🤖→👤 AI建议人类审批

## 输入

| 输入项 | 类型 | 必填 | 来源 | 说明 |
|--------|------|------|------|------|
| IA方案 | JSON | 是 | design-ia → ia.json | Pipeline 9输出的信息架构方案 |
| User Flow | JSON | 是 | design-userflow → userflow.json | Pipeline 10输出的用户流程 |
| 设计系统规范 | JSON/object | ○ | 用户提供 | 设计系统组件和规范定义 |

## 执行步骤

### Step 1: 低保真原型生成

生成线框图级别的原型描述：

- **布局**: 页面结构与区域划分
- **组件类型**: 按钮、表单、列表等
- **文案**: 关键文字内容
- **交互说明**: 基本交互行为

### Step 2: 中保真原型生成

在低保真基础上增加：

- **交互逻辑**: 点击、滑动等行为定义
- **状态变化**: normal/hover/active/disabled等
- **数据展示**: 列表数据、空状态、加载状态
- **过渡动画**: 页面切换和元素动画

### Step 3: 设计规范一致性检查

对照设计系统验证原型：

| 检查维度 | 说明 |
|----------|------|
| 组件来源 | 是否使用规范组件库 |
| 间距字号 | 间距和字体是否符合规范 |
| 颜色 | 色彩使用是否合规 |
| 交互模式 | 通用交互是否一致 |
| 文案风格 | 文案语气是否符合规范 |

生成一致性得分和violations列表。

### Step 4: 可用性启发式评估

基于Nielsen十大可用性原则评估：

1. 系统状态可见性
2. 系统与现实世界匹配
3. 用户控制与自由
4. 一致性与标准
5. 错误预防
6. 识别而非回忆
7. 使用的灵活性与效率
8. 美观且最小化设计
9. 帮助用户识别和恢复错误
10. 帮助与文档

评分范围0-10，**评分<3的弱项需提供改进建议**。

## 输出

**存储路径**：`output/pm-design/design-prototype/`

```json
{
  "prototype": {
    "fidelity": "medium",
    "pages": [
      {
        "name": "页面名称",
        "layout": {...},
        "components": [...],
        "interactions": [...],
        "states": {
          "default": {...},
          "loading": {...},
          "empty": {...},
          "error": {...}
        }
      }
    ],
    "design_system_compliance": {
      "score": 0.92,
      "violations": [
        {"item": "按钮颜色", "severity": "low"}
      ]
    },
    "heuristic_evaluation": {
      "overall_score": 8.5,
      "weak_areas": [
        {
          "principle": "系统状态可见性",
          "score": 2,
          "suggestion": "增加加载进度指示器"
        }
      ]
    }
  }
}
```

## 决策规则

- **设计规范一致性 < 0.85** 需人类确认violations
- **启发式评估弱项**（评分<3）需人类判断是否修改
- 弱项有具体改进建议才能通过

## 质量检查

- ✅ 设计规范一致性 ≥ 85%
- ✅ 启发式评估已完成
- ✅ 弱项有具体改进建议
- ✅ 所有核心页面已覆盖
- ✅ 关键交互状态完整

---

## 降级策略

当上游文件不存在时，本Skill仍可独立执行：

| 缺失的上游文件 | 降级方案 |
|---------------|---------|
| ia.json（IA方案） | 用户提供功能描述 → 生成低保真原型描述，标注"缺乏IA数据" |
| userflow.json（UserFlow方案） | 用户提供功能描述 → 生成低保真原型描述，标注"缺乏UserFlow数据" |
| 设计规范文件 | 用户提供功能描述 → 生成低保真原型描述，标注"缺乏设计规范" |
| IA + UserFlow + 设计规范 | 用户提供功能描述 → 生成低保真原型描述，整体置信度降低 |
| 所有上游文件均缺失 | 提示用户先执行前序阶段，或基于用户提供的功能描述生成低保真原型描述 |

数据获取说明：
- 本Skill需要IA、UserFlow和设计规范数据，请通过以下方式之一提供：
  1. 直接描述功能、页面结构和交互流程
  2. 上传ia.json / userflow.json / 设计规范文件
  3. 提供数据文件路径
- AI不负责外部数据采集，仅负责分析
