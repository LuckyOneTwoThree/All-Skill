...
description: SCAMPER结构化方案生成工具。基于HMW陈述，从替代、合并、改编、修改、转换用途、消除、逆向7个维度系统化生成产品方案。关键词：SCAMPER、方案生成、产品创新、结构化思考。
metadata:
  module: "产品构思与设计"
...
| 缺失的上游文件 | 降级方案 |
|---------------|---------|
| hmw.json（HMW陈述） | 用户提供问题和当前方案 → SCAMPER生成，标注"缺乏HMW结构化数据" |
| 当前方案描述 | 用户提供问题和当前方案 → SCAMPER生成，标注"缺乏当前方案数据" |
| 竞品方案数据（competitor-intel.json） | 用户提供问题和当前方案 → SCAMPER生成，标注"缺乏竞品方案参考" |
| hmw.json + 当前方案 + 竞品方案 | 用户提供问题和当前方案 → SCAMPER生成，整体置信度降低 |
| 所有上游文件均缺失 | 提示用户先执行前序阶段，或基于用户提供的问题和当前方案SCAMPER生成 |

数据获取说明：
- 本Skill需要HMW陈述、当前方案和竞品方案数据，请通过以下方式之一提供：
  1. 直接描述问题、当前方案和竞品方案
  2. 上传hmw.json / competitor-intel.json文件
  3. 提供数据文件路径
- AI不负责外部数据采集，仅负责分析
