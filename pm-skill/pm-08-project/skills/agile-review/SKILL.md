...
execution_depth:
  default: standard
  quick_description: "仅输出迭代评分和TOP3改进项"
  deep_description: "完整复盘 + 团队动力学分析 + 趋势对比 + 改进路线图"
---
...
### Step 1: Sprint评审 [核心]（评估目标达成情况并沉淀改进）

#### Step 1.1: 产出清单自动整理 [核心]
...
### Step 2: 复盘报告生成 [核心]（Sprint目标达成分析、交付质量评估、团队速率趋势、改进行动项）

Step 2接收Step 1的输出作为输入，结合Sprint计划和历史数据，生成完整的复盘报告。

#### Step 2.1: Sprint目标达成分析 [核心]
...
#### Step 3.2: 工程质量复盘 [条件]（Bug密度、技术债变化）
...
#### Step 3.3: 过程复盘 [深度]（Sprint效率、协作质量）
...
| 深度级别 | 输出范围 | 说明 |
|----------|----------|------|
| quick | 迭代评分 + TOP3改进项 | 核心结论 + 最小可行产物，仅输出Step 1.1产出清单和Step 2.1/2.2/2.4核心结论 |
| standard | 完整复盘报告（当前默认） | 完整产物，包含Step 1-2全部输出和Step 3（如有上线数据） |
