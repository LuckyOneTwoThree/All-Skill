...
execution_depth:
  default: standard
  quick_description: "执行风险状态监控和预警触发，输出当前风险清单与预警状态"
  deep_description: "完整升级处理流程 + 风险趋势分析 + 根因追溯 + 应急预案生成 + 风险复盘报告"
---
...
### Step 1: 风险监控（持续监控风险状态，检测风险指标变化与触发条件） [核心]
...
### Step 2: 升级处理（高优先级风险升级处理，启动应对流程与资源调配） [条件]
...
- [ ] 风险趋势分析覆盖至少3个周期（P1）
- [ ] 高风险项有跟进记录（P1）
- [ ] 升级路径与风险等级匹配（P0→总监级/P1→经理级/P2→组长级）（P0）
- [ ] 升级通知在风险等级对应SLA内发送（P0≤5分钟/P1≤30分钟/P2≤4小时）（P0）
- [ ] 升级原因包含≥3个要素（风险描述+影响范围+紧急程度）（P1）
- [ ] 升级超时有自动跟进机制（P0每15分钟/P1每2小时/P2每24小时）（P2）
- [ ] 100%的升级操作有记录且可追溯（P1）
