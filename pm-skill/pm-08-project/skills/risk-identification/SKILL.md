...
description: 当需要识别和评估项目风险时使用。风险自动识别与评估，持续扫描项目数据、外部数据和历史风险库，自动完成风险识别、评估、优先级排序、策略建议，输出完整的风险登记册。关键词：风险识别、风险评估、风险登记册、风险扫描、风险优先级、风险管理、会出什么问题、风险排查。仅输出 identified_risks（风险识别清单），不涉及跟踪和监控。
metadata:
  module: "项目管理与执行"
  sub-module: "风险管理"
  type: "pipeline"
...
execution_depth:
  default: standard
  quick_description: "直接输出风险清单和优先级排序"
  deep_description: "完整识别 + 风险量化评估 + 关联风险分析 + 风险应对路线图"
---
...
| 深度级别 | 输出范围 | 说明 |
|----------|----------|------|
| quick | 风险清单和优先级排序 | 核心结论 + 最小可行产物 |
| standard | 完整产物（当前默认） | 完整产物，包含全部Step输出 |
| deep | 完整识别 + 风险量化评估 + 关联风险分析 + 风险应对路线图 | 完整产物 + 扩展分析 + 深度推演 |
