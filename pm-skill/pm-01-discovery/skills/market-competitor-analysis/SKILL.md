...
description: 当需要进行综合竞品分析、竞品调研或竞争定位判断时使用。整合竞品情报采集、四象限定位和竞品报告。关键词：竞品分析、竞品调研、竞品情报、四象限、竞争对手。
metadata:
  module: "产品探索与发现"
  sub-module: "市场竞品"
  type: "pipeline"
...
- [ ] Feature Matrix已更新，变更已标注类型和影响程度（P1）
- [ ] 竞品口碑对比已完成（P1）
- [ ] 差异化机会已识别（P0）
- [ ] 定价策略对比已完成（P1）
- [ ] 战略方向推断已完成，低置信度已标注（P2）
- [ ] 告警已触发（影响程度≥4的变更）（P0）
- [ ] 数据来源已标注（P0）
- [ ] 关键发现已多源交叉验证（至少2个独立来源）（P0）
- [ ] 四象限已填充（直接/间接/替代/潜在）（P0）
- [ ] 每个象限至少1项（空象限已标注需补充）（P0）
- [ ] 每项标注了数据来源（data_source）（P0）
- [ ] 每项标注了置信度（confidence）（P1）
- [ ] 潜在竞品已标注需人类验证（needs_human_validation=true）（P1）
- [ ] 低置信度项已标注（P1）
- [ ] 空象限已标注需补充（非"无竞品"，而是"未识别到"）（P1）
- [ ] 象限间流动信号已标注（有信号时）（P2）
- [ ] 执行摘要包含3条核心发现+Top1策略（P0）
- [ ] 每个核心竞品有完整SWOT分析（P1）
- [ ] 竞争定位图已生成，含自身产品定位（P1）
- [ ] 护城河评估覆盖7个维度（P2）
- [ ] 差异化策略至少3条，每条有依据和优先级（P1）
- [ ] 所有推断标注了置信度（P1）
- [ ] 数据来源已列出（P0）
- [ ] Markdown报告格式完整，可直接交付（P0）
...
|---------------|---------|---------|------------|
| 竞品列表 | 用户提供品类关键词 → AI搜索识别竞品，标注"竞品列表为AI推断" | competitors[].name标注"AI推断"，strategic_signals.confidence上限降为0.5 | 要求用户提供竞品名称列表或品类关键词 |
| 所有上游文件均缺失 | 用户提供品类关键词 → 基于AI知识库搜索识别竞品并执行分析 | 全部推断标注"AI知识库推断"，needs_human_validation默认为true，告警仅纳入周报不触发即时通知 | 要求用户提供品类关键词和竞品名称列表 |
| monitor_config | 跳过该输入相关步骤，使用默认监控配置（扫描频率：每日，关注维度：全部，告警阈值：影响程度≥4） | 输出中不包含monitor_config相关定制化字段，告警阈值固定为≥4 | 要求用户提供监控频率、关注维度和告警阈值配置 |
| TAM/SOM数据缺失 | 市场概览章节标注"缺乏市场规模数据" | 市场概览章节不完整 | 要求用户提供市场规模数据或上传tam-som.json文件 |
| PEST数据缺失 | 跳过宏观环境章节 | 市场概览缺少宏观视角 | 要求用户提供宏观环境数据或上传pest.json文件 |
| 自身产品信息缺失 | 差异化策略标注为"通用建议" | 策略需结合自身情况调整 | 要求用户提供自身产品功能、定位和核心优势描述 |
| 若用户未提供category_keywords | 提示用户提供品类关键词，否则无法确定竞品分析范围 | 无法生成输出 | 要求用户提供品类关键词（如"在线教育""SaaS CRM"） |
