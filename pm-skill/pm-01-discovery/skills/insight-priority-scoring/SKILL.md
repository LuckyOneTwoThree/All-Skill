...
description: 当需要对需求列表进行加权优先级评分排序时使用。需求优先级自动评分，基于痛点强度、频率、可解决性和KANO系数计算综合分数，采用加权求和避免极端值。关键词：优先级评分、需求排序、KANO系数、痛点强度、可解决性、加权评分、先做哪个、最值得做、排优先级。
metadata:
  module: "产品探索与发现"
  sub-module: "需求洞察"
  type: "pipeline"
...
## 数据获取说明`n本Skill需要需求列表、KANO分类和痛点数据，请通过以下方式之一提供：
  1. 直接粘贴需求列表和优先级描述
