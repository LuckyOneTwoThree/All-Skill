...
description: 当需要将实验设计和执行结果汇总为完整可交付的A/B测试报告时使用。A/B测试报告自动生成，包含实验概述、统计结论、效果分析、异质性发现和行动建议。关键词：A/B测试报告、实验报告、统计结论、效果分析、实验总结、出个实验报告、AB测试总结、实验结果汇报。
metadata:
  module: "产品度量运营"
  sub-module: "实验验证"
  type: "pipeline"
...
| 输入项 | 类型 | 必填 | 来源 | 说明 |
|--------|------|------|------|------|
| 实验设计方案 | markdown | 是 | experiment-design | 假设、指标、分流方案、终止条件 |
| 实验执行结果 | markdown | 是 | experiment-execution | 统计结论、护栏指标、异质性效应、新奇效应 |
...
- 当核心指标显著正向且护栏安全时，推荐全量发布
- 当核心指标显著正向但护栏有风险时，推荐条件发布并附护栏专项优化方案
- 当核心指标不显著时，建议延长周期或扩大样本量
- 需要人类确认的决策点：全量发布决策、分群差异化发布策略、实验终止决策

## 降级策略

- 当无实验设计方案时：基于执行结果反推设计要素，标注"设计信息缺失"
- 当无实验执行结果时：仅生成设计方案摘要，标注"等待实验结果"
- 当统计结论不明确时：标注"结论待验证"，建议延长实验或增加样本量
- 数据不可用时：生成报告模板框架，统计结论章节标注"待数据"
