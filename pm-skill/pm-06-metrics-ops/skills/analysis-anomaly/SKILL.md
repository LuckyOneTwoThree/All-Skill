...
description: 自动化数据分析引擎。AI自动执行7×24小时运行，负责指标健康检查、异常检测、自动归因和洞察推送。当发现指标异常时输出完整的异常报告。关键词：异常检测、数据分析、自动归因、指标监控、异常报告。
metadata:
  module: "产品度量运营"
  sub-module: "数据分析"
  type: "pipeline"
...
| 输入项 | 类型 | 必填 | 来源 | 说明 |
|--------|------|------|------|------|
| 指标体系 | JSON | 是 | metrics-system → metrics.json | 需要监控的核心指标列表及定义 |
