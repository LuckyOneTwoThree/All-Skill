...
description: Sprint Planning自动化。将Product Backlog转化为Sprint Backlog，包含Sprint Goal建议、Story自动选取、工作量估算、容量匹配验证，输出完整的Sprint计划。关键词：Sprint规划、Sprint计划、迭代规划、Story选取、容量匹配。
metadata:
  module: "项目管理与执行"
  sub-module: "敏捷执行"
  type: "pipeline"
