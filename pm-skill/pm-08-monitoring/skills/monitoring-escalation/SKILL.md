...
1. **主动监控而非被动响应**：告警分级和升级自动执行，而非等待人工判断
2. **归因分层**：升级路径按告警级别分层，确保每级升级都有明确的触发条件和处理人
3. **决策规则前置**：升级规则和On-Call排班在系统建设时即配置，告警发生时自动执行
4. **持续学习**：升级处理结果反馈到知识库，持续优化升级规则
...
|---------------|---------|---------|---------|
| 告警数据 | 无法获取告警详情和级别 | 用户提供告警信息（现象、服务、时间），AI基于描述推断级别并生成升级建议 | 基于用户描述的升级建议 |
| On-Call 排班 | 无法确定通知接收人 | 用户提供当前值班人员联系方式，AI据此配置升级链 | 基于用户输入的升级链 |
| 告警规则 | 无法确定升级触发条件 | 使用默认升级规则（Critical 5min/High 15min/Medium 1h），标注需人工确认 | 基于默认规则的升级配置 |
| 知识库 | 无法提供历史处理参考 | 升级建议中不包含历史案例参考，标注"无历史案例" | 无历史参考的升级建议 |
