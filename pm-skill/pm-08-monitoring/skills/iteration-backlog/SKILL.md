...
1. **主动监控而非被动响应**：Backlog持续优化而非迭代前才整理
2. **归因分层**：按"问题优先级→关联分析→重组建议"分层推进
3. **决策规则前置**：优先级评分标准和重组规则在迭代启动时定义
...
| 监控告警 | JSON | ○ | monitoring-escalation → 告警数据 | 待解决的技术问题 |
...
|---------------|---------|---------|---------|
| 需求池 | 无法获取待排序的需求项 | 用户提供当前需求列表（标题+简述），AI基于用户输入重新排序 | 基于用户输入的优先级排序 |
| 技术债务 | 无法评估债务对Backlog的影响 | 跳过技术债务影响分析，优先级评分中债务权重置零 | 无债务影响的排序结果 |
| 监控告警 | 无法关联告警提升优先级 | 跳过告警关联分析，紧急优先策略不可用 | 无告警关联的排序结果 |
| 用户反馈 | 无法评估用户价值维度 | 用户价值评分基于需求描述推断，标注低置信度 | 低置信度的用户价值评分 |
