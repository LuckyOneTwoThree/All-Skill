...
description: 产品健康度自动诊断。采集多维度数据并进行综合评分、趋势预测和瓶颈识别，输出健康度报告。关键词：健康度评分、产品诊断、多维度评分、健康检查、产品健康。
metadata:
  module: "产品监控与迭代"
  sub-module: "问题诊断"
  type: "pipeline"
...
2. **归因分层**：健康度按"性能→可用性→满意度→业务"多维度分层评分，确保每层可独立追踪
3. **决策规则前置**：健康度评分偏差阈值和告警规则在系统初始化时定义
4. **持续学习**：诊断结论持续反馈，优化评分模型和权重
...
|---------------|---------|---------|---------|
| 性能数据 | 无法评估性能维度健康度 | 用户提供响应时间、吞吐量等关键性能指标数值，AI直接评分 | 性能维度直接评分结果 |
| 可用性数据 | 无法评估可用性维度健康度 | 用户提供SLA达成率、故障次数等指标，AI直接评分 | 可用性维度直接评分结果 |
| 用户满意度 | 无法评估满意度维度健康度 | 用户提供NPS/CSAT评分和投诉率数据，AI直接评分 | 满意度维度直接评分结果 |
| 业务指标 | 无法评估业务维度健康度 | 用户提供转化率、GMV、DAU/MAU、留存等指标，AI直接评分 | 业务维度直接评分结果 |
| 竞品动态 | 无法进行对标分析 | 跳过竞品对标，瓶颈识别仅基于自身数据 | 无竞品对标的健康度报告 |
