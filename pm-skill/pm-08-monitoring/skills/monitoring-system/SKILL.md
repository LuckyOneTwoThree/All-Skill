...
1. **主动监控而非被动响应**：持续扫描核心路径健康度，在异常萌芽阶段就介入，而非等告警来了再处理
2. **归因分层**：监控体系按"核心路径→指标→告警规则→On-Call手册"分层构建，确保每层可独立验证
3. **决策规则前置**：告警分级规则、升级阈值、On-Call响应策略在体系构建时就定义好
4. **持续学习**：每次告警处理结果反馈到知识库，持续优化告警规则
...
|---------------|---------|---------|---------|
| 指标体系 | 无法确定需监控的业务/技术指标 | 用户提供核心业务指标列表，基于通用指标模板补充黄金指标 | 基础监控指标配置 |
| 产品架构 | 无法识别核心路径和服务依赖 | 用户提供服务组件清单，按通用微服务架构推断依赖关系 | 基础核心路径清单 |
| SLA 要求 | 无法确定告警阈值和分级标准 | 用户提供关键服务的可用性目标，采用行业默认阈值（99.9%/99.5%/99%） | 基于默认阈值的告警规则 |
| 现有监控 | 无法复用已有配置 | 跳过兼容性检查，从零生成监控配置 | 全新监控配置 |
