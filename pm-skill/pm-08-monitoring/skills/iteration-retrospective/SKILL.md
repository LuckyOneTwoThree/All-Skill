...
1. **主动监控而非被动响应**：迭代结束自动触发复盘，而非等待人工发起
2. **归因分层**：按"数据收集→指标分析→问题识别→改进建议"分层推进
3. **决策规则前置**：复盘模板和关键指标阈值在迭代启动时定义
4. **持续学习**：复盘结论反馈到Backlog优化和迭代规划，形成持续改进循环
...
|---------------|---------|---------|---------|
| 迭代完成情况 | 无法自动获取交付数据 | 用户提供迭代完成情况（完成/未完成项、故事点），AI基于提供数据生成复盘 | 基于用户输入的复盘报告 |
| 质量指标 | 无法评估质量维度 | 用户提供缺陷数和返工情况，AI直接进行质量分析 | 基于用户输入的质量分析 |
| 团队反馈 | 无法获取团队主观评价 | 跳过协作分析维度，标注"缺少团队反馈数据" | 缺少协作维度的复盘 |
| 监控数据 | 无法评估稳定性变化 | 跳过监控数据分析，在复盘中标注"缺少稳定性数据" | 缺少稳定性维度的复盘 |
