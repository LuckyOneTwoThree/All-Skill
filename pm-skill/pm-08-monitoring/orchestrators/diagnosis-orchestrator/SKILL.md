...
6. **阶段总结**：所有子Skill执行完成后，生成阶段总结文档，写入 `output/phase-reports/pm-monitoring/diagnosis-orchestrator.md`
...
输入:
  ...
  competitor_intel: market-competitor-intel（可选）
  competitor_classification: market-competitor-quadrant（可选）
...
输入:
  ...
  retention_data: retention-churn（可选）
