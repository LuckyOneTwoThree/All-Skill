...
description: 当需要记录重要的架构或技术决策时使用。架构决策记录(ADR)自动生成，包含决策背景、备选方案评估、决策结果、影响分析和决策回顾机制。关键词：架构决策、ADR、技术决策、决策记录、Architecture Decision Record、方案评估、技术选型、拍板记录。
metadata:
  module: "产品开发与上线"
  sub-module: "开发交付"
  type: "pipeline"
...
| 输入项 | 类型 | 必填 | 来源 | 说明 |
|--------|------|------|------|------|
| 决策背景 | text | 是 | 用户输入 | 需要决策的问题、约束条件 |
| 备选方案 | text | 否 | 用户输入 | 已知的备选技术/方案 |
| 约束条件 | text | 否 | 用户输入 | 技术约束、团队约束、时间约束 |
...
- 当备选方案≥3且评估维度完整时，生成完整ADR含评分矩阵
- 当备选方案=2时，生成ADR但标注"建议补充第三方案以避免二选一偏见"
- 当决策涉及核心架构变更时，必须包含逆转成本分析
- 需要人类确认的决策点：最终方案选择、决策前提条件确认、回顾周期设定

## 降级策略

- 当无决策背景时：无法生成，要求用户提供决策问题
- 当无备选方案时：基于决策背景推导常见备选方案，标注"待方案补充"
- 当无约束条件时：基于行业通用约束评估，标注"约束待确认"
- 数据不可用时：生成ADR模板框架，关键评估字段标注"待补充"
