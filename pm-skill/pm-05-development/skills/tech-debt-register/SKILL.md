...
description: 当需要系统化管理技术债务或评估技术债务优先级时使用。技术债务登记册自动生成，包含债务识别与分类、影响评估、偿还优先级排序、偿还计划和预防策略。关键词：技术债务、技术债务登记、代码债务、架构债务、债务偿还、债务预防、技术欠账、代码债。
metadata:
  module: "产品开发与上线"
  sub-module: "开发交付"
  type: "pipeline"
...
| 输入项 | 类型 | 必填 | 来源 | 说明 |
|--------|------|------|------|------|
| 代码审查结果 | markdown | 否 | development-auto-review | 代码质量问题、架构违规 |
| PRD同步记录 | markdown | 否 | development-prd-sync | 需求变更导致的技术妥协 |
| 技术信息 | text | 是 | 用户输入 | 代码库状态、架构现状、团队反馈 |
...
- 当债务影响生产稳定性（评分≥4）时，优先级自动设为紧急
- 当复合利息为加速增长时，偿还优先级提升一级
- 当Sprint容量不足以偿还紧急债务时，必须上报并申请专项容量
- 需要人类确认的决策点：债务优先级调整、偿还时机选择、借债审批决策

## 降级策略

- 当无代码审查结果时：基于团队反馈识别债务，标注"待代码审查验证"
- 当无PRD同步记录时：跳过需求变更债务识别，标注"待PRD同步补充"
- 当债务影响评分无法量化时：采用定性评估，标注"评分待量化"
- 数据不可用时：基于用户提供信息生成债务登记框架，标注"待技术验证"
