...
description: 当需要从PRD和SRS中提取安全需求或制定产品安全标准时使用。产品安全需求清单自动生成，包含威胁建模、安全功能需求、数据保护需求、合规映射和安全验收标准。关键词：安全需求、威胁建模、数据保护、安全验收、安全合规、STRIDE、安全检查、安全评审。
metadata:
  module: "产品开发与上线"
  sub-module: "开发交付"
  type: "pipeline"
...
| 输入项 | 类型 | 必填 | 来源 | 说明 |
|--------|------|------|------|------|
| PRD | markdown | 是 | design-prd | 产品功能需求、用户场景、数据流 |
| SRS | markdown | 否 | requirements-srs | 功能需求编号、非功能需求、接口需求 |
| 隐私合规评估 | markdown | 否 | privacy-compliance-assessment | 个人数据清单、合规差距、整改建议 |
| 安全标准 | text | 否 | 用户输入 | OWASP Top 10、CIS、行业安全标准 |
...
- 当威胁评级为Critical/High时，对应安全需求优先级自动设为P0/P1
- 当产品涉及个人数据处理时，隐私合规评估为必填输入
- 当P0安全需求未通过验收时，阻止上线（安全卡口）
- 需要人类确认的决策点：威胁评级调整、安全需求优先级、合规框架选择、安全卡口豁免

## 降级策略

- 当无隐私合规评估时：安全需求不含隐私专项，标注"建议补充隐私合规评估"
- 当无安全标准时：默认采用OWASP Top 10 + CIS基准，标注"标准待确认"
- 当威胁建模数据不足时：基于已知攻击面生成基础威胁模型，标注"威胁面待完善"
- 数据不可用时：生成安全需求框架，威胁建模标注"待产品信息补充"
