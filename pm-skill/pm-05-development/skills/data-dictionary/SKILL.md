...
description: 当需要从PRD和SRS中提取数据实体定义或建立产品数据标准时使用。数据字典文档自动生成，包含数据实体定义、字段规格、数据关系、枚举值和业务规则。关键词：数据字典、数据定义、字段规格、数据实体、枚举值、业务规则、数据表结构、字段说明。
metadata:
  module: "产品开发与上线"
  sub-module: "开发交付"
  type: "pipeline"
...
| 输入项 | 类型 | 必填 | 来源 | 说明 |
|--------|------|------|------|------|
| PRD | markdown | 是 | design-prd | 产品功能需求、业务规则、用户场景 |
| SRS | markdown | 否 | requirements-srs | 功能需求编号、数据需求、接口需求 |
| 数据模型 | markdown | 否 | 用户输入 | 已有的ER模型或数据库Schema |
...
- 当PRD和SRS均完整时，生成完整数据字典含实体、关系、枚举和生命周期
- 当字段涉及敏感数据时，必须标注敏感度等级并关联安全需求
- 当枚举值超过20个时，建议改为数据表而非硬编码枚举
- 需要人类确认的决策点：实体边界划分、敏感度分级标准、数据保留期限

## 降级策略

- 当无SRS时：基于PRD推导数据实体，标注"待SRS确认"
- 当无数据模型时：从PRD业务逻辑推导实体和关系，标注"待数据模型验证"
- 当字段类型存在歧义时：采用最通用类型，标注"类型待确认"
- 数据不可用时：生成数据字典框架，核心实体标注"待需求确认"
