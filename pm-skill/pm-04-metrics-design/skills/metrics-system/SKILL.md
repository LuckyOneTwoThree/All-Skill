---
name: metrics-system
description: 指标体系自动构建。包含北极星指标校验与推荐、L1/L2指标拆解、行动指标识别、虚荣指标检测。关键词：指标体系、AARRR模型、北极星指标、L1/L2指标、OSM模型、度量体系。
metadata:
  module: "产品度量设计"
  sub-module: "指标体系"
  type: "pipeline"
...
**自动补充逻辑**：
```python
def supplement_l2_metrics(l1_layer, current_l2_count):
    if current_l2_count < 3:
        recommended = get_default_l2_for_layer(l1_layer)
        return recommended[0:3-current_l2_count]
    return []
```
