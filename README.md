...
> 这套 Skill 体系将产品方法论结构化为可执行的 Pipeline，让 AI 成为方法论的**忠实执行者**——但它永远无法替代人的**判断力**（至少目前来说）。

| AI 擅长的 | 人必须掌控的 |
|:----------|:------------|
| 规模化数据处理、结构化分析、模式识别、方案穷举、一致性检查 | 战略取舍的平衡点、用户痛点的优先级判定、商业假设的真伪裁决、体验细节的审美判断、风险边界的最终拍板 |

每个编排器的阶段卡口和人类决策点，不是流程的冗余，而是**人机协作的分界线**。跳过它们，AI 会高效地走向错误的方向。

**好的产品，始终是人的判断力 × AI 的执行力。**
...
兼容 Trae / Claude Code 的 Agent Skills 开放标准。每个 Skill 是一个可独立执行的方法论 Pipeline，编排器（Orchestrator）负责调度子 Skill 的执行顺序和阶段卡口。四大领域通过**数据契约**紧密衔接，形成从产品探索到上线运营的完整闭环。

> 源目录统计以 `skills-manifest.json` 为准，`codex/skills` 作为 Codex 部署目标。
...
└──────┼─────────┼─────────┼─────────┼────────────────────────────────────┘
       ...
       ▼         ▼         ▼         ▼
┌──────────────────────────────┐  ┌──────────────────────────────────────┐
│  🎨 UI 设计与前端（正确地呈现） │  │  ⚙️ 后端架构与开发（先全量设计再实现）  │
│                              │  │                                      │
│  项目初始化 → 页面构建 →      │  │  ┌─ 设计 ─ 架构→数据→API ──┐         │
│  API集成 → 生产就绪          │  │  │    统一设计审查          │         │
│                              │  │  └─ 实现 ─ 数据→API→架构 ──┘         │
│  project-init                │  │                                      │
│  page-builder                │  │  backend-orchestrator（顶层编排）     │
│  api-integration             │  │  api-design（安全+认证+合规）         │
│  production-ready            │  │  data-architecture（缓存+迁移）       │
│                              │  │  backend-architecture（审查+ADR）     │
│                              │  │                                      │
│                              │  │  双输出：代码→{project_dir}/src/      │
│                              │  │        元数据→output/                 │
└──────────────────────────────┘  └──────────────────────────────────────┘
```
...
| **架构方案+拓扑图** | backend backend-architecture-spec | backend data-architecture-spec / api-design-spec | 架构决策约束数据模型和API设计 |
| **服务数据归属** | backend backend-architecture-spec | backend data-architecture-spec | 定义每个服务拥有的数据实体，驱动数据建模 |
| **技术栈决策** | backend backend-architecture-spec | backend 三个impl Skill | 统一所有实现Skill的技术栈 |
| **数据模型** | backend data-architecture-spec | backend api-design-spec | 数据模型是API设计的基础（v5.0必填输入） |
...
│   ├── orchestrators/                     backend-orchestrator（顶层两阶段编排器）
│   ├── backend-01-api-design/             模块1：API设计（数据驱动契约，字段有据可依）
│   │   ├── orchestrators/                     api-design-orchestrator
│   │   └── skills/                            api-design-spec + api-design-impl
│   ├── backend-02-data-architecture/       模块2：数据架构（架构约束先行，模型决定上限）
...
│
├── codex/                             🔄 Codex 部署目标 —— 扁平化英文 Skill 集
│   └── skills/                            120个扁平化Skill（英文版，含Codex运行时特化）
│
├── codex-templates/                   🔄 Codex 模板 —— 编写规范与协议
│   ├── orchestrator-protocol.md           Codex编排器协议（双模式调用+artifact-index）
│   ├── orchestrator-skill-template/       Codex编排器Skill编写模板
│   ├── pipeline-skill-template/           Codex Pipeline Skill编写模板
│   ├── engineering-boundary-protocol.md   工程交付边界协议
│   └── execution-depth-protocol.md        执行深度协议
│
└── cross-domain/                      ✅ Skill 文件 —— 跨领域协调
    └── orchestrators/                     product-iteration-orchestrator / product-launch-orchestrator
```

> **Skill 提取规则**：只有标记 ✅ 的目录下包含可部署的 Skill 文件。每个 Skill 的最小单元是 `{skill-name}/SKILL.md`，部署时只需将最内层的 `{skill-name}/` 文件夹扁平复制到 `.trae/skills/` 下。`templates/`、`scripts/`、`.github/` 等为项目基础设施，不需要部署。标记 🔄 的 `codex/` 和 `codex-templates/` 为 Codex 部署目标，已扁平化且含运行时特化，直接复制 `codex/skills/` 下的文件夹即可。
...
> **先全量设计，再统一实现**：Backend 模块采用两阶段模式——设计阶段按架构→数据→API顺序产出设计规范，三份设计交叉验证后统一审查；实现阶段按数据→API→架构顺序生成可运行代码。顶层编排器（backend-orchestrator）协调两阶段流程，三个子编排器可独立调用。

#### 顶层编排器

| 编排器 | 作用 | 调度策略 |
|:-------|:-----|:---------|
| backend-orchestrator | 后端全流程两阶段编排 | 阶段A：架构→数据→API设计 → 统一设计审查 → 阶段B：数据→API→架构实现 |

#### 模块1：后端架构（设计先行）

架构服务业务，简单方案优先，按需演进。架构决策在最前，约束后续数据模型和API设计。

| Skill | 作用 | 关键衔接 |
|:------|:-----|:---------|
| backend-architecture-spec | 架构模式→ADR→服务设计→服务数据归属→技术栈决策→后端审查 | **输入**：pm PRD → **输出**：架构方案 + service_data_ownership.json + tech_stack_decision.json → **人类审查** |
| backend-architecture-impl | app.ts+配置→服务层→基础设施→Docker+CI→统一对齐检查→测试生成 | **输入**：backend-architecture-spec 产出 + api-design-impl + data-architecture-impl → **输出**：代码写入{project_dir}/ |

#### 模块2：数据架构（架构约束先行）

架构约束先行，模型决定上限，缓存决定下限，迁移可回滚。

| Skill | 作用 | 关键衔接 |
|:------|:-----|:---------|
| data-architecture-spec | 数据字典→ER建模→表结构索引→缓存策略→迁移方案 | **输入**：pm PRD + 架构方案 + 服务数据归属 + 技术栈决策 → **输出**：er_model.json + 缓存策略 + 迁移方案 → **人类审查** |
| data-architecture-impl | Model→Migration→Repository→缓存层→对齐检查→测试生成 | **输入**：data-architecture-spec 产出 + 技术栈决策 → **输出**：代码写入{project_dir}/src/ |

#### 模块3：API设计（数据驱动契约）

数据驱动契约，字段有据可依，安全内建而非外挂。

| Skill | 作用 | 关键衔接 |
|:------|:-----|:---------|
| api-design-spec | 资源识别→接口设计→安全设计→认证鉴权→合规检查 | **输入**：pm PRD + 架构方案 + 服务设计 + 技术栈决策 + ER模型（必填） → **输出**：openapi.yaml + 安全策略 + 认证鉴权方案 → **人类审查** |
| api-design-impl | 代码骨架→Service实现→中间件→对齐检查→测试生成 | **输入**：api-design-spec 产出 + 数据模型 + 数据层实现报告 → **输出**：代码写入{project_dir}/src/ ← 前后端核心契约 |
...
PM探索发现 → PM商业战略 → PM构思设计(PRD) ──┬── UI项目初始化 → UI页面构建 → UI API集成 → UI生产就绪
                                              │                                         ↑
                                              └── 架构设计→数据设计→API设计 → 数据实现→API实现→架构实现
                                                       ↑
                                                 PRD+架构方案+数据模型
...
需要外部数据时，用户需通过**粘贴 / 上传 / 提供路径**三种方式提供。
...
| 需要设计API | Backend模块3 api-design-orchestrator（数据驱动契约） |
| 需要设计数据库 | Backend模块2 data-architecture-orchestrator（架构约束先行） |
| 需要确定架构模式 | Backend模块1 backend-architecture-orchestrator |
| 需要后端全流程 | Backend backend-orchestrator（顶层两阶段编排） |
...
| 资源 | 链接 |
|:-----|:-----|
| 📋 贡献指南 | [CONTRIBUTING.md](CONTRIBUTING.md) —— 命名规范、编写模板、PR 流程 |
| 🗺️ 路线图 | [ROADMAP.md](ROADMAP.md) —— 待认领 Skill 和新领域扩展方向 |
| 🐛 提交 Issue | [报告 Skill 执行问题](https://github.com/LuckyOneTwoThree/All-Skill/issues/new?template=bug-skill.yml) |
| 💡 提议新 Skill | [提案新增方法论 Pipeline](https://github.com/LuckyOneTwoThree/All-Skill/issues/new?template=new-skill.yml) |
