...
description: "Use when conducting comprehensive competitor analysis. Integrates competitor intelligence collection, four-quadrant positioning, and competitor reporting. Keywords: competitor analysis, competitor research, competitor intelligence, four-quadrant, competitive landscape."
metadata:
...
1. **Multi-source cross-validation** -- Single-source competitor intelligence is unreliable; each key finding must be cross-validated by at least 2 independent sources; strategic inference confidence is highest when hiring + financing + feature update signals converge
2. **Change equals signal** -- Every feature change, pricing adjustment, and hiring change by competitors is a strategic signal, not an isolated event; must be interpreted in context rather than simply listed
3. **Tiered alert response** -- P0 (impact >= 5) urgent notification + mark for emergency response; P1 (impact = 4) immediate notification + include in weekly report; P2 (impact < 4) include in weekly report only; resource allocation matches impact level
...
5. **Four-quadrant definition first** -- Direct/indirect/substitute/potential quadrants have strict definitions (same category + same users + same features / same scenario + different solution / non-productized method / capable of entering); classification must be based on definitions, not intuition
6. **Inter-quadrant flow traceable** -- Competitors are not statically assigned to one quadrant; indirect competitors may upgrade to direct; potential competitors may become direct; annotate flow signals and estimated timelines
7. **Potential competitors default to validation required** -- Every item in the potential competitor quadrant defaults to needs_human_validation=true, because potential competitor identification is based on inferred signals (hiring/patents/financing) with the highest uncertainty
8. **Empty quadrant equals risk alert** -- Any empty quadrant is not "no competitors" but "no competitors identified"; must annotate that the quadrant needs supplementation and suggest human provide leads
9. **Data-driven conclusions first** -- Every conclusion must be supported by data or evidence; baseless inference is prohibited
10. **Structured output is deliverable** -- Reports are for decision-makers, not for AI; readability is prioritized
11. **Insight over data stacking** -- Data is the means, insight is the purpose; every piece of data must answer "so what"
12. **Actionable recommendations** -- Strategy recommendations must be specific enough for "what to do + why + expected effect"
...
|--------|---------|---------|
| App version update monitoring | Version number, changelog, feature changes, release date | Each version release |
...
- Large-scale hiring for a specific tech stack -> Infer technology direction investment
- New overseas positions -> Infer internationalization strategy
- Sharp decrease in hiring -> Infer cost contraction or strategic adjustment
...
| Step | Description |
|------|------|
| Detect version update | Obtain version update information from collection layer |
...
| Assess impact level | 1-5 score for impact on competitive landscape |
| Trigger alert | Impact level >= 4 triggers real-time alert |

**Change type definitions:**
- Added: Competitor adds a previously unavailable feature
- Upgraded: Competitor makes significant improvements to an existing feature
- Removed: Competitor discontinues a feature
- Downgraded: Competitor restricts or downgrades an existing feature

#### Competitor User Reputation Comparison

| Analysis Dimension | Description |
|---------|------|
| Sentiment distribution comparison | Positive/neutral/negative sentiment ratio comparison across competitors |
...
| Competitive disadvantage alerts | Identify reputation disadvantage items relative to competitors |
...
| Analysis Dimension | Description |
|---------|------|
| Price range comparison | Pricing range and average price comparison across competitors |
| Plan structure comparison | Free/Basic/Pro/Enterprise feature distribution comparison |
...
3. Search same-category products through product directories/industry databases
4. SEO competitor analysis (competitors bidding on same core keywords)
...
**Identification logic:**
1. Analyze target user scenarios, list all possible solution methods
...
4. Identify products solving the same scenario but with different technology paths/business models
...
**Definition:** Users' current non-productized solution methods

**Identification logic:**
1. Identify target users' solution methods when no product in this category exists
2. Extract current workflows/manual processes from user interview data
3. Collect non-productized alternative methods from surveys
...
| Data Source | Collection Content | Reliability |
|--------|---------|--------|
| User interview data | Current solution methods described by users | High |
| Survey questionnaires | Alternative methods selected by users | High |
| Forums/communities | DIY solutions, manual process discussions | Medium |
| Industry reports | Proportion of non-productized solutions in the industry | Medium |
...
**Identification logic:**
1. Job posting monitoring: Detect hiring for related technology/marketing positions
2. Patent analysis: Search patent applications in related technology fields
3. Financing information: Monitor companies receiving financing in related fields
4. Strategic announcements: Analyze related directions mentioned in company strategies

**Data sources:**

| Data Source | Collection Content | Reliability |
|--------|---------|--------|
| Job postings | Related technology/marketing position hiring | Low-Medium |
| Patent databases | Related technology patent applications | Medium |
| Financing information | Financing events in related fields | Medium |
| Strategic announcements | Related directions mentioned in company strategies | Low-Medium |
| Supply chain analysis | Upstream/downstream enterprise extension capabilities | Low |

#### Confidence Assessment and Human Validation Annotation

For each item in each quadrant, perform confidence assessment:

| Assessment Dimension | Description |
|---------|------|
| Data source reliability | Credibility of data sources (0-1) |
| Evidence sufficiency | Quantity and quality of evidence supporting the classification |
| Classification certainty | Degree of certainty for placing the competitor in this quadrant |
...
- Low (<0.5): Inferred conclusions, require human verification
...
- Each competitor may optionally include `flow_signal` (flow signal description) and `estimated_flow_timeline` (estimated flow timeline)
...
Integrate Step 1 and Step 2 data, construct complete profiles for each core competitor:
...
| Team and financing | User provided / AI inferred | Team size, financing rounds, capital reserves |
...
|------|---------|------|
| S+O | Growth strategy | Leverage strengths to seize opportunities |
| W+O | Improvement strategy | Address weaknesses to seize opportunities |
| S+T | Defense strategy | Leverage strengths to counter threats |
| W+T | Crisis contingency | Response when weaknesses encounter threats |

#### Competitive Positioning Map (Perceptual Map)

Draw competitive positioning map based on two core dimensions:

**Dimension selection rules:**
- Prioritize the 2 dimensions users care about most in their decisions
...
| Technology-focused | Technology advancement | Ecosystem maturity |
...
|-----------|---------|---------|
| Network effects | Does user growth enhance product value | 0-5 points |
| Switching costs | Cost for users to migrate to competitors | 0-5 points |
| Scale economy | Does scale bring cost advantages | 0-5 points |
| Brand barrier | Brand awareness and trust | 0-5 points |
| Technology barrier | Irreproducibility of core technology | 0-5 points |
...
**Moat depth rating:**
- Total score >= 25: Deep moat (difficult to displace)
...
| Bottom-up | User count/revenue data for each competitor available | Competitor public data |
...
Based on preceding analysis, generate differentiation strategy recommendations:
...
| Competitors with shallow moats | Flanking breakthrough strategy: Enter from the weakest-moat competitor |
...
| Fragmented market landscape | Focus strategy: Focus on one segment and go deep in a fragmented market |
...
Assemble all analysis into a complete Markdown report:
...
- Top pain points horizontal comparison
...
```json
{
  ...
  "properties": {
    ...
    "reputation_comparison": {"type": "object", "description": "Competitor reputation horizontal comparison"},
...
| competitors | array | Yes | Must contain at least 1 competitor entry; each must include name and category |
| competitors[].feature_matrix | object | Yes | Must include features array and last_updated timestamp |
| competitors[].feature_matrix.features | array | Yes | Each item must include feature_name, status, impact_degree, source |
...
| competitors[].reputation.sentiment_distribution | object | Yes | Sum of positive+neutral+negative must equal 1.0 (+/-0.01 tolerance) |
| competitors[].reputation.data_sources | array | Yes | At least 1 reputation data source must be annotated |
...
| competitors[].strategic_signals.evidence | array | Yes | Must include at least 1 evidence; each must annotate source type and confidence |
...
| quadrants.potential_competitors.items[].needs_human_validation | boolean | Yes | Whether human validation needed; **default must be true** |
| quadrant_summary | object | Yes | Classification statistical summary |
| quadrant_summary.total_items | number | Yes | Total competitors, should equal sum of items across four quadrants |
| quadrant_summary.by_confidence | object | Yes | Statistics by confidence level |
| quadrant_summary.by_confidence.high | number | Yes | High confidence items (>= 0.8) |
| quadrant_summary.by_confidence.medium | number | Yes | Medium confidence items (0.5-0.8) |
| quadrant_summary.by_confidence.low | number | Yes | Low confidence items (< 0.5) |
| quadrant_summary.needs_validation_count | number | Yes | Number of items needing human validation |
...
| executive_summary.competition_landscape | string | Yes | One-paragraph competitive landscape summary, >= 50 characters |
...
| Strategic inference escalation | Competitor strategic inference confidence < 0.5 | Escalate to human judgment, annotate as needing validation |
...
| Potential competitor validation needed | Any item in potential competitor quadrant | Default annotate needs_human_validation=true; confidence typically low, requires human confirmation |
...
| Moat assessment data insufficient | Moat assessment stage | Annotate confidence for each dimension; provide inference basis for low-confidence dimensions |
...
- [ ] Alerts triggered (changes with impact level >= 4)
- [ ] Data sources annotated
- [ ] Key findings cross-validated by multiple sources (at least 2 independent sources)
- [ ] Four quadrants filled (direct/indirect/substitute/potential)
- [ ] At least 1 item per quadrant (empty quadrants annotated as needing supplementation)
- [ ] Each item annotated with data source (data_source)
- [ ] Each item annotated with confidence (confidence)
- [ ] Potential competitors annotated as needing human validation (needs_human_validation=true)
- [ ] Low confidence items annotated
- [ ] Empty quadrants annotated as needing supplementation (not "no competitors" but "not identified")
- [ ] Inter-quadrant flow signals annotated (when signals exist)
- [ ] Executive summary contains 3 key findings + Top 1 strategy
- [ ] Each core competitor has complete SWOT analysis
- [ ] Competitive positioning map generated, including own product positioning
- [ ] Moat assessment covers 7 dimensions
- [ ] At least 3 differentiation strategies, each with basis and priority
...
|---------|---------|---------------|---------|
| pest.json | Policy/regulatory changes | Affects competitor compliance cost assessment, may change compliance cost dimension in pricing strategy comparison | Re-evaluate affected competitors' pricing.value_score, update compliance-related inferences in strategic_signals |
| pest.json | Technology dynamics changes | Affects competitor technology direction judgment, may change impact assessment of technology features in Feature Matrix | Re-evaluate impact_degree of related feature changes, update technology direction inferences in strategic_signals |
| tam-som.json | Market size data changes | Affects competitive landscape assessment, may change market expansion/contraction judgment in competitor strategic inference | Re-evaluate competitors' strategic_signals.direction, adjust confidence values |
| tam-som.json | Segment market data changes | Affects competitor target customer migration direction inference | Update customer migration-related inferences in strategic_signals, re-evaluate differentiation opportunities |

### Downstream Notification Mechanism Table

| This Skill Output Change | Notify Downstream Skill | Notification Content | Trigger Condition |
|---------------|-------------|---------|---------|
| Differentiation strategy change | design-orchestrator | Changed strategy name, adjustment direction, new priority | Strategy added/removed/priority adjusted |
