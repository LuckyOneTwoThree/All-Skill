---
name: page-builder
description: "Use when generating frontend pages and components. Integrated page and component construction, generating components within page context based on visual direction and design tokens, assembling into complete pages with built-in quality gates ensuring output quality. Keywords: page generation, component generation, page assembly, UI construction, build pages, create components, assemble pages."
metadata:
...
1. Scan first: identify existing framework, component library, styling solution, router, state management, and design tokens before writing code; inherit by default.
2. Target scope: declare target directories and files before implementation; generated code must stay inside {project_dir}/src/ unless integration files are explicitly required.
3. No overwrite: preserve existing user code, components, pages, and configurations unless the user explicitly asks for replacement.
4. Token-first: all style values must reference design tokens; no hardcoded colors, font sizes, or spacing values.
5. Accessibility baseline: every interactive component must include ARIA attributes, keyboard navigation, and focus management.
...
- Userflow's data_operations define the data operation types and associated entities that pages must support, but component selection, data fetching methods, and state management solutions are decided by this Skill
...
| visual_policy | string | O | output/ui-project-init/project-init.json -> visual_direction.visual_policy | Visual strategy; default existing-design-system-first |
...
- If PRD input exists: Its defined functional areas and content requirements serve as **must-cover feature checklist** (pages must include all functional areas defined in PRD), but area layout method, visual hierarchy, spacing rhythm is determined by page-builder combined with visual_direction
- PRD defines "what functional areas the page needs", page-builder decides "how these areas are visually presented"
...
- When alternative type better matches visual direction (e.g. when visual_direction.tension_level=bold, Card Grid is more suitable than Table), prefer alternative type
...
| Design Tension | Visual boldness degree: conservative->safe restrained, balanced->balanced, bold->impactful, extreme->maximal experimentation | tension_level |
| Visual Narrative | Page eye flow path (e.g. "Z-pattern reading -> focus CTA -> progressively reveal details") | visual_narrative |
...
| shadow_style | Shadow style must be consistent: none->no shadows, flat->offset only no blur, subtle->slight spread, elevated->multi-layer spread, dramatic->large projection | Shadow style inconsistent with definition |
| spacing_rhythm | Spacing must follow rhythm pattern: tight->4px base, standard->8px base, relaxed->16px base; regular->even increment, jazz->jumping(4/8/24/8), symphonic->multi-level(4/8/16/32/48) | Spacing not matching rhythm pattern |
...
| brand_color_usage | Brand color distribution must match: accent->buttons/links/icons only, spotlight->key area backgrounds+CTA, flood->large area backgrounds+gradients+Hero area | Brand color distribution inconsistent with definition |
| image_treatment | Image treatment must be consistent: none->text only, photography->real photos, illustration->illustrations, 3d->3D renders, abstract->abstract graphics, minimal-icon->minimalist icons | Image style inconsistent with definition |
| motion_style | Motion intensity must match: none->no motion, subtle->micro-feedback(color change/micro-shift), moderate->smooth transitions(slide-in/fade-in), expressive->spring+choreography(stagger/spring), theatrical->dramatic choreography(full-screen transitions/particles) | Motion intensity inconsistent with definition |
| grid_density | Information density must match: sparse->generous+large whitespace(<=3 content blocks per screen), balanced->standard spacing(4-6 content blocks per screen), dense->compact+information-dense(>=7 content blocks per screen) | Content block count inconsistent with density definition |
...
**Override Consumption Example**:

```
Global anchors: grid_density=balanced, brand_color_usage=accent
Page "Landing" override: grid_density->dense, brand_color_usage->spotlight
-> Landing page consumption: grid_density=dense, brand_color_usage=spotlight (remaining 6 dimensions use global values)
-> Other pages consumption: all 8 dimensions use global values
```

**tension_level Visual Pattern Library**:

tension_level determines overall visual boldness degree; different levels correspond to different visual implementation strategies:
...
| extreme | Experimental layout, full-screen visuals, overlapping elements, unconventional grid | Brand color dominant (>50%), clashing colors, high saturation | Extreme jumps (2x+), text overlay on images, unconventional typography | Extreme spacing contrast (4px<->64px), whitespace and density alternating | Brand color flood mode, full-screen gradients + text overlay |
...
**PM Constraint Deviation Recording** (design freedom safeguard mechanism):

When page-builder's design judgment is inconsistent with PM input constraints, deviation decisions must be explicitly recorded, ensuring deviations are traceable, assessable, and reversible:

**Exploration Phase Decision Consumption Rules**:
- If exploration phase design decisions input exists (design_decisions.json from Stage 1 conditional branch): Its decisions[] serve as design_decisions initial values
- page-builder continues to append new deviation records on top of exploration phase decisions
- Exploration phase confirmed decisions (severity=minor/moderate and human confirmed) are not re-recorded
- Exploration phase unconfirmed decisions (severity=major/critical) need re-evaluation and confirmation
...
Component mapping: Iterate through page functional requirements, match components from component library item by item, annotate inter-component data dependencies and interaction communication methods.

**Design Brief Consumption** (consuming design_brief.json):

When design_brief.json exists, page-builder must consume it as a **strong constraint input**, not optional reference. Design brief contains executable design specifications produced by ext Skills, directly guiding code generation:

| Consumption Dimension | design_brief Field | Consumption Method | Priority |
|----------|------------------|---------|--------|
| Color Specification | color_specifications | Specific CSS color values directly applied (e.g. `oklch(97% 0.01 60)`), replacing derived values from tokens | Strong constraint |
...
| Layout Instructions | layout_instructions | Layout type and visual focus as design reference, page-builder can adjust layout implementation based on page context and visual_direction (e.g. changing recommended sidebar-main to tab-main, must record in design_decisions) | Guiding |
...
| Animation Specification | animation_specifications | Animation parameters (duration/easing/delay) directly applied | Strong constraint |
| Brand Color Strategy | brand_color_strategy | Brand color distribution areas and proportions directly followed | Strong constraint |
| Visual Bans | visual_bans | Banned patterns/styles absolutely not appearing | Strong constraint |
| Differentiation Direction | differentiation_direction | Aesthetic differentiation direction as design decision basis | Guiding |

**Consumption Rules**:
- Specific values in design_brief (CSS color values, font sizes, spacing values) **override** derived values in visual_direction
- Layout instructions in design_brief serve as **design reference**, page-builder can adjust implementation based on page context, adjustments recorded in design_decisions
...
- When guiding dimensions in design_brief (layout_instructions/differentiation_direction) conflict with page-builder's design judgment: Design judgment takes precedence, deviations recorded in design_decisions
...
**Design Brief Driven**: When design_brief.component_specifications exists, implement according to specified component structure/states/variants/specific_values directly, rather than deriving from component library default configuration.
...
- If interaction-spec input exists: State machines must **cover** all states and transitions defined in interaction spec (completeness constraint), but state machine visual expression, transition animations, interaction details are decided by page-builder (implementation freedom)
- If interaction-spec input exists: Exception paths in interaction spec (network errors/insufficient permissions/empty data etc.) must be mapped to component states, no omissions allowed
- If interaction-spec input exists: Accessibility interaction requirements in interaction spec (focus management/ARIA labeling/keyboard operations) must be implemented

Animation intent consumption (interaction-spec defines intent, page-builder + ext-interaction-design decides implementation):

| Scenario | interaction-spec Intent Example | page-builder Implementation Decision |
|------|-------------------------|---------------------|
| Modal expand | "Modal expand should have elastic feel" | Specific easing function and duration decided by ext-interaction-design combined with visual_direction |
| List loading | "Loading process should have progress indication" | Skeleton/Spinner/Progress bar decided by page-builder combined with visual_direction |
| Action feedback | "Successful action should have feedback" | Toast/Inline Message/Animation decided by page-builder combined with visual_direction |

If interaction-spec doesn't define animation intent, use page-builder built-in default animation specification table. Specific animation values in interaction-spec (easing functions/duration/thresholds) are for reference only; page-builder has the right to adjust based on visual_direction.
...
| Check Item | Pass Standard | Level |
|--------|---------|------|
| Visual rhythm compliance | Page implementation consistent with Step 1 visual rhythm design (6 dimensions all reflected: visual focus/density distribution/color rhythm/layering/design tension/visual narrative) | P0 |
| Brand color proportion | Brand color proportion within visual_direction.color_strategy corresponding range (Restrained:5-15%/Committed:30-60%/Full palette:15-40%/Drenched:>60%) | P1 |
...
| visual_direction consistency | Component visual style consistent with aesthetic_direction description, no style characteristics not mentioned in aesthetic_direction | P0 |
| Whitespace rhythm | Page spacing non-uniform distribution, at least 3 different spacing values forming rhythm | P1 |
| Visual anchor compliance | 8 anchor dimensions (border_radius_level/shadow_style/spacing_rhythm/type_scale/brand_color_usage/image_treatment/motion_style/grid_density) implementation consistent with visual_direction definition | P0 |
| AI homogenization feature detection | Execute per visual_policy: under existing-design-system-first, homogenization features are hints only, must not override existing design system; under balanced, treated as P1; under differentiation-strict, treated as P0 | P0/P1 |
| Visual boredom detection | Not satisfying any of the following is "visually boring": (1)brand color proportion <5% (2)all font size jumps <1.25x (3)all spacing values identical (4)no visual focus area (5)no shadows or depth layering | P1 |
...
| Page manifest coverage | All page_ids in page_manifest.json.pages[] have corresponding pages generated (when no page_manifest, check prd.json.pages[]) | P0 |
  ...
  "properties": {
    ...
    "quality_report": {
      ...
      "properties": {
        ...
        "aesthetic_score": {"type": "number", "description": "Aesthetic score (0-100), calculation: audit percentile x0.5 + critique percentile x0.5 (calculated by orchestrator stage-4 and backfilled); built-in self-assessment score when audit not executed"}
      }
    },
    "design_decisions": {
      "type": "array",
...
```json
{
  ...
  "suggestions": [{
    ...
    "current_value": "3 independent functional areas: user info/order list/action panel",
    "suggested_value": "Merged into 1 Tab-style area, 3 tabs switching display",
    "rationale": "User task flow is coherent, separate areas increase cognitive load and page vertical length, Tab approach better matches operation flow",
    "impact_assessment": {
      "affected_downstream": ["ia_proposals.json (routes may simplify)", "component_catalog.json (component type change)"],
      "breaking_change": false,
      "effort": "low"
    },
    "priority": "medium"
  }],
  "summary": "Recommend merging user center page 3 independent functional areas into Tab-style layout"
}
```
...
- [ ] Color value references 100% use Token variables
...
- [ ] AI homogenization check compliant with visual_policy; by default must not break existing design system due to single font or card grid characteristic
...
- [ ] Visual boredom detection passed (brand color proportion >=5%, font size jumps >=1.25x, spacing has variation, has visual focus, has depth layering)
- [ ] Audit design taste score >=75
- [ ] Page structure recommendation has been data-driven reviewed (by orchestrator calling ext-ui-ux-pro-max)
- [ ] Differentiation suggestions applied (by orchestrator calling ext-frontend-design)
- [ ] Interaction design applied (by orchestrator calling ext-interaction-design, when not purely static components)
- [ ] ext-impeccable sub-command enhancements applied (by orchestrator calling)
